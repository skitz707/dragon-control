...
<div id="mainContent">
	...
	<table class="standardResultTable" style="width: 35%; margin-left: auto; margin-right: auto;">
		<tr>
		...
		<tr>
			<td>Proficiencies</td>
			<td><?php echo getProficienciesCheckboxes($database, $character); ?></td>
		</tr>
		<tr>
...
// print condition types checkbox
//-------------------------------------------------------------------------------------------
function getProficienciesCheckboxes($database, $character) {
	$selectStmt = "select * from dragons.proficiencies order by proficiency";
	$returnHTML = "";
	$boxesPerLine = 3;
	$i = 1;
	
	$characterProficiencies = $character->getCharacterProficiencies();
	...
	if ($selectHandle = $database->databaseConnection->prepare($selectStmt)) {
		...
		while ($data = $selectHandle->fetch(PDO::FETCH_ASSOC)) {
			...
			if (in_array($data['proficiencyId'], $characterProficiencies)) {
				$checked = " checked";
			} else {
				$checked = "";
			}
			
			$returnHTML .= '<td><input type="checkbox" name="proficiencyIds[]" id="' . $data['proficiency'] . '" value="' . $data['proficiencyId'] . '"' . $checked. ' /></td>';
			$returnHTML .= '<td>' . $data['proficiency'] . '</td>';
